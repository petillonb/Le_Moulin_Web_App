...
export class SupabaseService {
  ...
  async fetchEventDataByActiviteId(activiteId: number): Promise<EventActivite[]>{
    console.log("fetching event", activiteId);
    const eventQuery = await this.supabase
      .from('event')
      .select('id,date,activite_id(id,nom,secteur,enfant,adulte)')
    ...
    console.log(event);
    return (event  as EventActivite[]);
  }

  async fetchParticipantDataByEventId(eventId: number): Promise<Participant[]>{
    const participantQuery = await this.supabase
    .from('participant')
    .select('id, present, identite_id(id,nom,prenom,date_naissance,nationalite,genre,contact_id(id,mobile,fixe,mail,adresse)),event_id(id,date,activite_id(id,nom,secteur,enfant,adulte))')
    .eq('event_id',eventId)
  ...
  async updateActiviteData(activiteData: Activite){
    console.log("updating table", activiteData);
  ...
  async supprimerParticipantData(id: number){
    const response = await this.supabase
    .from('participant')
    .delete()
    .eq('id', id);
  ...
  async supprimerEventData(id: number){
    const response = await this.supabase
    .from('event')
    .delete()
    .eq('id', id);
  ...
  async insertActiviteData(activiteData: Activite){
    console.log("inserting activite:", activiteData);
    const activiteDataQuery= await this.supabase
      .from('activite')
      .insert({ 
        nom: activiteData.nom,
        secteur: activiteData.secteur,
      ...
      .select()
    type activiteData= QueryData<typeof activiteDataQuery>
    console.log("test")
    const { data, error } = await activiteDataQuery
    if (error) throw error
    const activite: activiteData = data as activiteData
    console.log("test")
    return (activite[0]  as Activite);
  ...
  async insertEventData(eventData: EventActivite){
    console.log("inserting event:", eventData);
    console.log(typeof eventData.date);
    const { } = await this.supabase
      .from('event')
      .insert({
        activite_id: eventData.activite_id,
        date: eventData.date
      })
    }


  async insertParticipantData(participantData: Participant){
    console.log("inserting participant:", participantData)
    const { } = await this.supabase
    .from('participant')
    .insert({
      identite_id: participantData.identite_id.id,
      event_id: participantData.event_id.id,
  ...
  async supprimerActiviteData(id: number){
    const response = await this.supabase
    .from('activite')
    .delete()
    .eq('id', id);
  ...
  async setParticipantPresent(identiteId: number){
    console.log("Setting Present", identiteId);
    const { } = await this.supabase
      .from('participant')
      .update({
        present: true,
      })
      .eq('identite_id',identiteId);
      console.log("updated table");

  }
  async setParticipantAbsent(identiteId: number){
    console.log("Setting Present", identiteId);
    const { } = await this.supabase
      .from('participant')
      .update({
